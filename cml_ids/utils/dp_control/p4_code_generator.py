...
class P4CodeGenerator():
    """"Generate P4 code snippets.

    Attributes
    ----------
    rf_path: str
        Path of random forest classifier/estimator.
    features_path: str
        Path for all used flow-based features updated in P4 switch and used for classification.
    p4_base_path: str
        Base path for saveing P4 code snippets.
    features_ids_path: str
        Path for the features with ids.
    relevant_features: list
        Features used for training ML models.
    """

    def __init__(self, rf_path, features_path, p4_base_path, features_ids_path, relevant_features) -> None:
        self.rf_path = rf_path
        self.features_path = features_path
        self.p4_base_path = p4_base_path
        self.feature_to_id_dict = {}
        self.id_to_feature_dict = {}
        self.features_ids_path = features_ids_path
        self.relevant_features = relevant_features
        self._initialize()
        print("Started P4 code generator.")

    def _feature_map(self, feature, id):
        """Function for mapping from feature name to feature id, vice versa.

        Parameters
        ----------
        feature: str
            Flow-based feature name.
        id: str
            Flow-based feature id.
        """
        self.feature_to_id_dict[feature] = id
        self.id_to_feature_dict[id] = feature

    def _initialize(self):
        """Initialize dictornaries.
        """
        # dicts for mapping features to ids and vice verse.
        df = pd.read_csv(self.features_ids_path)
        df.apply(lambda x: self._feature_map(x[0], x[1]), axis=1)
        # load all used features
        self.features = pd.read_csv(self.features_path)["feature_name"].tolist()
        # load RF
        with open(self.rf_path, "rb") as f:
            self.rf_estimator = pickle.load(f)
    ...
    def generate_struct_to_bitstring_action(self):
        """Generate P4 code for converting struct to bitstring.
        """
        output_str = '''
        /**
        ...
        **/
        ...
        fh = open(self.p4_base_path + file_name, 'w')
        fh.write(output_str)
        fh.close()
        print("Generated struct to bistring code.")

    def generate_bitstring_to_struct_action(self):
        """Generate P4 code for converting bitstring to struct.
        """
        ...
        for feature in self.features:
            ...
            if ("ms" in feature):
                ...
                raise TypeError("Feature name is illegal.")
    ...
    def generate_compare_feature_action(self):
        ...
        file_name = f"p4_compare_feature_action_feature_num_{len(self.relevant_features)}.txt"
        fh = open(self.p4_base_path + file_name, 'w')
        fh.write(output_str)
        fh.close()
    ...
    def generate_mathch_action_tables(self):
        ...
        file_name = f"p4_match_action_tables_action_depth_{trees[i].max_depth}.txt"
        fh = open(self.p4_base_path + file_name, 'w')
        fh.write(output_str)
        fh.close()
    ...
    def generate_classfication_logic(self, with_trace=False):
        ...
        if with_trace:
            for i in range(len(trees)):
                ...
                file_name = f"p4_random_forest_classification_logic_depth_{trees[i].max_depth}_with_trace.txt"
        else:
            for i in range(len(trees)):
                tree_index = i + 1
                output_str = output_str + f'''
            ...
            file_name = f"p4_random_forest_classification_logic_depth_{trees[i].max_depth}.txt"
    ...
    def generate_p4_rules(self):
        ...
        max_depth = self.rf_estimator.estimators_[i].max_depth
        file_name = f"rf_rules_depth_{max_depth}_feature_num_{len(self.relevant_features)}.txt"
        fh = open(self.p4_base_path + file_name, 'w')
        fh.write(rules)
        fh.close()
